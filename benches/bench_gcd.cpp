...
#else
	tr.m_dut.rootp->SBGCDCtrl__DOT__regA = a;
	tr.m_dut.rootp->SBGCDCtrl__DOT__regB = b;
#endif
...
void interface_tlm_write(gcd_tlm &dut_tlm) {
...
void interface_tlm_read(gcd_tlm &dut_tlm) {
	uint32_t address = klee_int("address");
...
void interface_rtl_write(gcd_test_runner &tr) {
	uint32_t address = klee_int("address");
	...
	assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regReadyBuf == 0);
	assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regResBuf == 0);

	if((address&0xFF) == addrValid)
		assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regValid == (value==1));
	else
		assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regValid == 0);

	if((address&0xFF) == addrA) {
		assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regA == value);
	} else {
		assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regA == 0);
	}
	if((address&0xFF) == addrB) {
		assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regB == value);
	} else {
		assert(tr.m_dut.rootp->SBGCDCtrl__DOT__regB == 0);
	}
...
void interface_rtl_read(gcd_test_runner &tr) {
	uint32_t address = klee_int("address");
