...
void tlm_write(hash_tlm &tlm, uint32_t address, uint8_t *val) {
...
struct functional_tlm_basic : public sc_core::sc_module {
	...
	functional_tlm_basic(sc_core::sc_module_name name, hash_tlm& dut) : sc_module(name), tlm(dut) {
	...
	void run() {
		...
		wait(tlm.irq);
		assert(tlm.hash == hash_reference(a, b));
...
struct functional_rtl_basic : public sc_core::sc_module {
	...
	functional_rtl_basic(sc_core::sc_module_name name, sc_clock &clock)
			: sc_core::sc_module(name), rtl{"hash_rtl"}, clock(clock), bus(clock) {
		rtl.clk(clock);
	...
	void run() {
		...
		bus.write(addrB, b);
#else
		rtl.rootp->SBTaskHash__DOT__regA = a;
		rtl.rootp->SBTaskHash__DOT__regB = b;
#endif
...
struct comparison_basic : public sc_core::sc_module {
	...
	comparison_basic(sc_core::sc_module_name name, sc_clock &clock, hash_tlm &tlm) : sc_module(name), clock(clock), tlm(tlm), rtl{"hash_rtl"}, bus(clock) {
		rtl.clk(clock);
...
struct interface_tlm_write : public sc_core::sc_module {
	...
	interface_tlm_write(sc_core::sc_module_name name, hash_tlm& dut) : sc_module(name), tlm(dut) {
...
struct interface_tlm_read : public sc_core::sc_module {
	...
	interface_tlm_read(sc_core::sc_module_name name, hash_tlm& dut) : sc_module(name), tlm(dut) {
...
struct interface_rtl_write : public sc_core::sc_module {
	...
	void run() {
		...
		if((address&0xFF) == addrValid)
			assert(rtl.rootp->SBTaskHash__DOT__regValid == (value==1));
		else
			assert(rtl.rootp->SBTaskHash__DOT__regValid == 0);
		if((address&0xFF) == addrA) {
			assert(rtl.rootp->SBTaskHash__DOT__regA == value);
		} else {
			assert(rtl.rootp->SBTaskHash__DOT__regA == 0);
		}
		if((address&0xFF) == addrB) {
			assert(rtl.rootp->SBTaskHash__DOT__regB == value);
		} else {
			assert(rtl.rootp->SBTaskHash__DOT__regB == 0);
		}
		assert(rtl.rootp->SBTaskHash__DOT__regReadyBuf == 0);
		assert(rtl.rootp->SBTaskHash__DOT__regHashBuf == 0);
...
int sc_main(int argc, char* argv[])
{
	if(argc == 2)
	{
		...
		{
			sc_clock clk{"hash_clk", sc_core::sc_time(20, sc_core::SC_NS)};
			functional_rtl_basic testbench("name", clk);
			sc_start(sc_time(690, sc_core::SC_NS));
			assert(testbench.test_done && "test case unexpectedly did not finish");
